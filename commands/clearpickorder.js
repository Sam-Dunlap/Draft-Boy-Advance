	...
	async execute(interaction) {
		...
		if (cache) {
			await interaction.reply(
				"Clearing the pick order while in a live draft is a recipe for disaster. You should take a long, hard look at yourself, and then try running this command again once the draft is complete."
			);
			return;
		}
		const drafterRole = interaction.guild.roles.cache.find(
			role => role.name === "Drafter"
		);
