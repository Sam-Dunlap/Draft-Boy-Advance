...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('continue')
        .setDescription('pick the draft up from where it left off. use only if the bot crashed mid-draft')
    ...
    async execute(interaction) {
        ...
        const outputChannel = await interaction.guild.channels.fetch(outputChannelId);
        

        const cache = new Cache(outputChannelId, pickOrder, x, interaction.guild.id, teamSize);
        saveCache(cache);
        ...
        return outputChannel.send(`${userMention(drafterId)}, you're up next. Lock in a pokemon with /pick.`);
